...
mod tests {
    ...
    #[tokio::test]
    async fn test_receipt_root_task_ignores_invalid_index_for_bloom_aggregation() {
        ...
        let (tx, rx) = bounded(4);
        let handle = ReceiptRootTaskHandle::new(rx);
        let join_handle = tokio::task::spawn_blocking(move || handle.run(1));

        tx.send(IndexedReceipt::new(0, valid)).unwrap();
        tx.send(IndexedReceipt::new(999, invalid)).unwrap();
        drop(tx);
